...
void Ringbuf_init(void)
{
  ...
  __HAL_UART_ENABLE_IT(uart_1, UART_IT_ERR);
  __HAL_UART_ENABLE_IT(uart_2, UART_IT_ERR);
	__HAL_UART_ENABLE_IT(uart_3, UART_IT_ERR);

  /* Enable the UART Data Register not empty Interrupt */
  __HAL_UART_ENABLE_IT(uart_1, UART_IT_RXNE);
  __HAL_UART_ENABLE_IT(uart_2, UART_IT_RXNE);
	__HAL_UART_ENABLE_IT(uart_3, UART_IT_RXNE);
}
...
void Uart_write(int c, UART_HandleTypeDef *uart)
{
	if (c>=0)
	{
		...
		__HAL_UART_ENABLE_IT(uart_1, UART_IT_TXE); // Enable UART transmission interrupt
		}
			...
			__HAL_UART_ENABLE_IT(uart_2, UART_IT_TXE); // Enable UART transmission interrupt
			}
		else if (uart == uart_3){
			...
			__HAL_UART_ENABLE_IT(uart_3, UART_IT_TXE); // Enable UART transmission interrupt
			}
	}
}
...
{
	if (uart == uart_1) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer1->head - _rx_buffer1->tail) % UART_BUFFER_SIZE;
	else if (uart == uart_2) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer2->head - _rx_buffer2->tail) % UART_BUFFER_SIZE;
	else if (uart == uart_3) return (uint16_t)(UART_BUFFER_SIZE + _rx_buffer3->head - _rx_buffer3->tail) % UART_BUFFER_SIZE;
	return -1;
}
...
int16_t Get_position (char *string, UART_HandleTypeDef *uart)
{
  ...
  if (uart == uart_1)
  {
	 ...
	 {
	  if (Uart_read(uart_1) == string[so_far])
	  		{
  ...
  else if (uart == uart_2)
  {
	 ...
	 {
	  if (Uart_read(uart_2) == string[so_far])
	  		{
	  		  counter=UART_BUFFER_SIZE-1;
	  		  so_far++;
	  		}
